...
<div class="container">
  <h4 class="title is-4">Add text via API</h4>
      ...
      <span class="is-size-6 is-bold ml-2 monospace">http://textlooker.com/api/text</span><br />
      <span class="is-size-6 header"> Content-Type :&nbsp</span> 
      <span class="is-size-6 has-text-primary"> application/json</span> 
          <pre class='mt-3'>
              {@html syntaxHighlight(requestObject)}
            </pre>
</div>
...
<style type='scss' global>
  pre {
    ...
    span.number { color: darkorange !important; }
